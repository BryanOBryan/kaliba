...
class Uri implements UriInterface
{
     ...
     * @param string|array $query    Uri query string.
     * @param string $fragment Uri fragment.
     * @param string $user     Uri user.
     * @param string $password Uri password.
     */
    public function __construct($scheme,$host,$port = null,$path = '/',$query = null,$fragment = '',$user = '',$password = '') 
    {
     ...
     *
     * @param array|ServerBag $server
     *
     * @return self
     */
    public static function createFromServer($server)
    {
        if(is_array($server)){
            $env = new ServerBag($server);
        }
        if(is_object($server) && $server instanceof ServerBag){
            $env = $server;
        }
        // Scheme
    ...
    /**
     * Create a full URL Strings
     * @param string $location
     * @param mixed $data
     * @return self
     */
    public static function makeUrl($location, $data=null)
    {
        $query = [];
        ...
        $uri = static::createFromGlobal();
        $baseUrl = $uri->getBaseUrl();
        return static::createFrom($baseUrl.'/'.$location, $query);
    }
    
     /**
     * Create new Uri Global
     * @return static
     */
    public static function createFromGlobal()
    {
        return static::createFromServer($_SERVER);
    }
     ...
     */
    public static function createFrom($url, $query = [])
    {
        if (!empty($query)) {
            $q = (strpos($url, '?') === false) ? '?' : '&';
            $url .= $q;
     ...
     *
     * @param string|array $query The raw uri query string.
